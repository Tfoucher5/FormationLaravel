...
@section('title', __('create_motif'))

@section('content')
    <div class="max-w-md mx-auto mt-10 bg-white shadow-lg rounded-lg p-6">
        <h1 class="text-2xl font-bold mb-6 text-center text-gray-700">{{ __('create_motif') }}</h1>
        ...
        <form action="{{ route('motif.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input value="{{ old('libelle') }}" type="text" name="libelle" id="libelle" class="w-full px-4 py-2 mt-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="{{ __('motif_name') }}" required>
                @error('libelle')
